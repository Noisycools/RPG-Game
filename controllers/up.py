...
class UpButton(Image):
	...
	def __init__(self, **kw):
		...
		self.pos = (dp(CONTROLLER_SIZE), dp(CONTROLLER_SIZE))
		self.source = "assets/controller/UP.png"

	def on_touch_down(self, touch):
		"""On Touch Down"""
		if self.collide_point(*touch.pos):
			self.parent.touched = False
			if self.parent.walk_down or self.moving: return False
	...
	def animate_walk(self):
		...
		if self.player_pic == "left":
			self.parent.look_position = "up"
			self.parent.player.source = "assets/character/player/look_up_right.zip"
			self.player_pic = "right"
		else:
			self.parent.look_position = "up"
			self.parent.player.source = "assets/character/player/look_up_left.zip"
			self.player_pic = "left"
		self.parent.player_head.source = "assets/character/playerhead/up.png"
		self.parent.player_head.source = "assets/character/playerhead/up_walk.zip"

	def activate_walk(self, touch):
		"""Activate Walk"""
		touch.grab(self)
		self.moving = True
		self.parent.walk_up = True
	...
	def condition(self):
		"""Condition"""
		if (self.parent.right_button.moving is False
	...
	def check_restriction(self):
		"""Check Restriction"""
		if self.parent.pos_row-1 < 0: return True
	...
	def walk_animation(self, touch):
		...
		if self.moving is False or self.parent.down_button.moving: return False
		self.parent.player.anim_delay = WALK_SPEED if self.parent.running is False else RUN_SPEED
		self.parent.player_head.anim_delay = self.parent.player.anim_delay
...
#			anim = Animation(y=widget.y-self.parent.block_size,
#				d=(WALK_SPEED*2 if self.parent.running is False else RUN_SPEED*2), t="linear")
#			anim.start(widget)
